...
def render_frontal_views_for_one_face(
    ...
    """
    Render frontalized views for one face.
    """
    mkdir(out_dir)

    vertices = ver_to_vertices(ver_dense)
    faces = tri_to_faces(tri)

    if R is not None:
        vertices = frontalize_vertices(vertices, np.asarray(R, dtype=np.float64), frontal_rot=frontal_rot)
    else:
        # Fallback: if pose extraction failed, just center+normalize without frontal rotation
        vertices = vertices - vertices.mean(axis=0, keepdims=True)

    vertices = normalize_vertices(vertices, target_extent=1.8)

    vertices[:, 1] *= -1  # Flip Y for better visualization (OpenGL camera looks at -Z, so +Y is up)
    faces = faces[:, [0, 2, 1]]
